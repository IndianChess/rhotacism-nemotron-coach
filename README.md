...
| Coach LLM (hosted Space) | `nvidia/Llama-3.1-Nemotron-Nano-8B-v1` (via Featherless AI) | 8 B | HF Inference router |
| Voice (TTS) | `pocket-tts` | ~250 M | local |

> ⚠️ **About the hosted Space right now:** the public Space is using the **HF Inference router (`nvidia/Llama-3.1-Nemotron-Nano-8B-v1` via Featherless AI)** for coach replies because the CPU-tier Space can't run the 4 B GGUF in-process fast enough. Flip `COACH_BACKEND=local` to use the **4 B Nemotron local model**, which is what qualifies the project for the Tiny Titan badge. See "Coach backend" below.
...
Calls `nvidia/Llama-3.1-Nemotron-Nano-8B-v1` through the HF Inference router, pinned to the Featherless AI provider (`:featherless-ai` suffix on the model ID). Needs `HF_TOKEN` set (Space secret, or `.env` locally). Fast and reliable, but pulls inference off-device, so this mode does *not* qualify for the Tiny Titan badge. To use a different served model, set `COACH_MODEL_ID` (and include the `:provider-slug` suffix if the model only has one provider).
